...
class ApiClientService {
  ...
  func fetchData(endpoint: String, city: String) async -> FutureForecastModel? {
    guard let url = URL(string: "\(baseUrl)\(endpoint).json?lang=en&key=\(API_KEY)&q=\(city)&days=14") else {
      debugPrint("⚠️", "Can't parse url!", "⚠️")
      return nil
    }
    ...
    do {
      ...
      debugPrint("⚠️", "Fetching or decoding data failed!", "⚠️")
      debugPrint("⚠️", error, "⚠️")
      return nil
    }
  }
}
